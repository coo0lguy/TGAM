...
STATICS_QUERY_PATH = os.path.join(SQL_DIR, 'statics_new_final.sql')
...
if __name__ == '__main__':
    ...
    ap.add_argument('--out_path', type=str, default= './Extract_sepsis3_final',
    ...
    ap.add_argument('--psql_schema_name', type=str, default='public,mimiciii,cohort',
                    help='Postgres database name.')
    ...
    if (args['extract_numerics'] == 0 | (args['extract_numerics'] == 1) ) & isfile(os.path.join(outPath, dynamic_hd5_filename)):
        ...
        select distinct i.subject_id, i.hadm_id, i.icustay_id, o.charttime, o.itemid, cast(o.value as character varying), valueuom
        FROM icustay_detail i
        INNER JOIN outputevents o ON i.hadm_id = o.hadm_id
        where i.icustay_id in ({icuids})
          and o.itemid in ({ouitem})
          and o.charttime between (intime - interval '6' hour) and outtime
