...
STATICS_QUERY_PATH = os.path.join(SQL_DIR, 'statics_new_final_icd9.sql')
...
def fill_missing_times(df_by_sid_hid_itemid):
    ...
    filler = pd.DataFrame({'subject_id':[sid]*len(missing_hours),
                           ...
                           'stay_id':[icustay_id]*len(missing_hours),
...
def save_numerics(
    ...
    X['value'] = pd.to_numeric(X['value'], 'coerce')    # 使用NaN代替非数字数据
    X.astype({k: int for k in ID_COLS})

    to_hours = lambda x: max(0, x.days*24 + x.seconds // 3600)

    X = X.set_index('stay_id').join(data[['intime']])
    ...
    X = X.join(var_map).join(I).set_index(['label', 'LEVEL1', 'LEVEL2'], append=True)   # icustayid,itemid,label,LEVEL1,LEVEL2,subjectid,hadmid,value,valueuom,dbsource,linkto,category,unitname
    standardize_units(X, name_col='LEVEL1', inplace=True)   # 统一单位

    if apply_var_limit > 0: 
        X = apply_variable_limits(X, var_ranges, 'LEVEL2')

    group_item_cols = ['LEVEL2'] if group_by_level2 else ITEM_COLS
    X = X.groupby(ID_COLS + group_item_cols + ['hours_in']).agg(['mean', 'std', 'count'])   # 得到数据为三个关键点就是这里
...
def save_outcome(
    ...
    for task in tasks:
        ...
        new_data = new_data.reset_index()
        Y = Y.merge(
            new_data[['subject_id', 'hadm_id', 'stay_id', 'hours_in', task]],
            on=['subject_id', 'hadm_id', 'stay_id', 'hours_in'],
            how='left'
        )
...
if __name__ == '__main__':
    ...
    ap.add_argument('--out_path', type=str, default= './Extract_sepsis3_final_icd9',
                    help='Enter the path you want the output')
    ap.add_argument('--resource_path',type=str,default=os.path.expandvars("./resources/"))
    ap.add_argument('--queries_path',type=str,default=os.path.expandvars("./SQL_Queries/"))
    # 是否提取人口统计数据
    ...
    ap.add_argument('--psql_schema_name', type=str, default='public,mimiciv_derived,mimiciv_hosp,mimiciv_icu',
                    help='Postgres database name.')
    ap.add_argument('--psql_user', type=str, default='postgres',
                    help='Postgres user.')
    ap.add_argument('--psql_password', type=str, default='psql123',
    ...
    ap.add_argument('--min_age', type=int, default=16,
                    help='Minimum age of patients to be included')
    ap.add_argument('--min_duration', type=int, default=24,
    ...
    static_data_schema = load_datapackage_schema(
        os.path.join(args['resource_path'], 'static_data_spec_new_final.json'))
    outcome_data_schema = load_datapackage_schema(
        os.path.join(args['resource_path'], 'outcome_data_spec_new.json'))
    ...
    if (args['extract_numerics'] == 0 | (args['extract_numerics'] == 1) ) & isfile(os.path.join(outPath, dynamic_hd5_filename)):
        ...
        outputitems_to_keep = var_map.loc[var_map['LEVEL2'] == 'Urine output'].ITEMID
        outputitems_to_keep = set([ str(i) for i in outputitems_to_keep ])

        # 手动核对得到的变量chartitems的itemid和labitems的itemid
        ...
        where c.stay_id in ({icuids})
          ...
          and c.charttime between icu_intime and icu_outtime
        ...
        where i.stay_id in ({icuids})
          ...
          and l.charttime between (icu_intime - interval '6' hour) and icu_outtime
          and l.valuenum > 0 -- lab values cannot be 0 and cannot be negative
          
        UNION ALL

        select distinct i.subject_id, i.hadm_id, i.stay_id, o.charttime, o.itemid, cast(o.value as character varying), valueuom
        FROM icustay_detail i
        INNER JOIN outputevents o ON i.hadm_id = o.hadm_id
        where i.stay_id in ({icuids})
          and o.itemid in ({ouitem})
          and o.charttime between (icu_intime - interval '6' hour) and icu_outtime
          and o.value is not null
          -- and o.valuenum > 0 -- output values cannot be 0 and cannot be negative
        ;
        """.format(icuids=','.join(icuids_to_keep), chitem=','.join(chartitems_to_keep_manual), lbitem=','.join(labitems_to_keep_manual), ouitem=','.join(outputitems_to_keep))
